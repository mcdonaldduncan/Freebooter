    ...
    void Update()
    {
        switch (m_TurretState)
        {
            case TurretState.GUARD:
                Tracking.LimitedTrackTarget();
                if (Tracking.CheckFieldOfView()) Invoke(nameof(SetAttackState), m_AttackDelay);
                if (IsInCombat) HandleCombatStateChange();
                break;
            case TurretState.ATTACK:
                Tracking.LimitedTrackTarget();
                if (Tracking.CheckFieldOfView()) Shooting.Shoot(true);
                if (!Tracking.InRange) m_TurretState = TurretState.GUARD;
                if (!IsInCombat) HandleCombatStateChange();
                break;
            default:
                break;
